...
def cv_analyze_rgb(img_bgr):
    ...
    return dict(vegetation_pct=veg_pct, bare_pct=bare_pct, burn_pct=burn_pct,
...
def cv_change_detect(img_a, img_b):
    ...
    regions  = len([c for c in contours if cv2.contourArea(c) > total*0.005])

    return dict(change_pct=change_pct,
